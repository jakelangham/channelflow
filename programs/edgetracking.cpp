...
int EdgeStateTracking(FlowField& uL, FlowField& uH, const EdgetrackingFlags& etflags, const AttractorFlags& aflags,
                      const TimeStep& dt, const DNSFlags& dnsflags, ostream& os);

void CreateFiles(const EdgetrackingFlags& etflags);
Real Bisection(FlowField& uL, FlowField& uH, Real t0, const EdgetrackingFlags& etflags, const AttractorFlags& aflags,
               const TimeStep& dt, const DNSFlags& dnsflags, ostream& os = std::cout);  // Return: lambdaH
bool AdvanceLH(FlowField& uL0, FlowField& uH0, int& t0, const EdgetrackingFlags& etflags, const TimeStep& dt,
               const DNSFlags& flags, const AttractorFlags& aflags, ostream& os, ConvergenceData& cd);
void ChooseNewLH(const FlowField& u0, Real& lambdaL, Real& lambdaH, const Real t, const EdgetrackingFlags& etflags,
                 const AttractorFlags& aflags, const TimeStep& dt, const DNSFlags& dnsflags, ostream& os = std::cout);
void SaveForContinuation(const FlowField& uL, const FlowField& uH, const string dirBisections, Real t);

Attractor CalcAttractor(const FlowField& u0, FlowField& returnField, const AttractorFlags& aflags, const TimeStep& dt,
                        const DNSFlags& flags, const string energyfile, const string saveFinalFF = "",
                        const int tMin = 0, const int tMax = 10000, bool verbose = false, const int t0 = 0,
                        std::ostream& os = std::cout);
Attractor CalcAttractor(const FlowField& u0, const AttractorFlags& aflags, const TimeStep& dt, const DNSFlags& flags,
...
Attractor attractor(const FlowField& u, const AttractorFlags& aflags);
...
int main(int argc, char* argv[]) {
    cfMPI_Init(&argc, &argv);
    {
        ...
        ArgList args(argc, argv, "Find edge state");
        ...
        const string ifname = (cont) ? "" : args.getstr(1, "initial flowfield", "initial flowfield");
        ...
        ostream& os = (ostream&)*dnsflags.logstream;
        ...
        if (!cont) {
            CreateFiles(etflags);
            u = FlowField(ifname, cfmpi);
            uH = u;  // temp for calculating projection error and optimizing FFTW
            if (dnsflags.symmetries.length() > 0) {
                printout("Projectin FlowField to invariant subspace generated by symmetries", os);
                stringstream sstr2;
                sstr2 << dnsflags.symmetries;
                printout(sstr2.str(), os);
                u.project(dnsflags.symmetries);
            ...
            fftw_savewisdom();
            if (chooseLHfirst) {
                ChooseNewLH(u, lambdaL, lambdaH, 0., etflags, aflags, dt, dnsflags, os);
            }
            uH = u;
            uH *= lambdaH;
            uL = u;
            ...
            uH.project(dnsflags.symmetries);
            uL = FlowField(fffileL, cfmpi);
            uL.project(dnsflags.symmetries);
            u = uH;
            uH.optimizeFFTW(FFTW_PATIENT);
            uH = u;
            ...
            if (etflags.tLastWrite >= dnsflags.T - 1) {
                cferror("Already over time limit");
            }
        }

        EdgeStateTracking(uL, uH, etflags, aflags, dt, dnsflags, os);
    }
    cfMPI_Finalize();
    return 0;
}
...
*/
int EdgeStateTracking(FlowField& uL_, FlowField& uH_, const EdgetrackingFlags& etflags, const AttractorFlags& aflags,
                      const TimeStep& dt, const DNSFlags& dnsflags, ostream& os) {
    int t = dnsflags.t0;
    ...
    while (!finished && t <= dnsflags.T) {
        ...
        Bisection(uL, uH, t, etflags, aflags, dt, dnsflags, os);

        // Step 2: Advance FlowFields
        printout("t = " + i2s((int)t) + ": Advancing flowfields");
        bool finished = AdvanceLH(uL, uH, t, etflags, dt, dnsflags, aflags, os, convergencedata);
        if (!finished) {
            if (etflags.keepUL) {
                u = uL;
            } else {
            ...
            ChooseNewLH(u, lambdaL, lambdaH, t, etflags, aflags, dt, dnsflags, os);
            uL = u;
            uL *= lambdaL;
            uH = u;
            uH *= lambdaH;
...
void CreateFiles(const EdgetrackingFlags& etflags) {
    ...
    if (taskid == 0) {
        ...
        fL << fieldstatsheader_t() << endl;
        fH << fieldstatsheader_t() << endl;
        fL.close();
        fH.close();
...
Real Bisection(FlowField& uL, FlowField& uH, Real t0, const EdgetrackingFlags& etflags, const AttractorFlags& aflags,
    ...
    SaveForContinuation(uL, uH, etflags.directoryBisections, t0);
    while (l2d > etflags.epsilonBisection) {
        ...
        if (nTraj == 1) {
            ...
            string f = "";
            Attractor a = CalcAttractor(uT, aflags, dt, dnsflags,
 ...
 * @return t, final time
 */
bool AdvanceLH(FlowField& uL0, FlowField& uH0, int& t, const EdgetrackingFlags& etflags, const TimeStep& dt,
               const DNSFlags& flags, const AttractorFlags& aflags, ostream& os, ConvergenceData& cd) {
    // Create TimeStep, pressureField and DNS objects
    //     Real t = t0; // Actual time
    int t0 = t;

    vector<FlowField> fieldsH(2);
    vector<FlowField> fieldsL(2);
    fieldsL[0] = uL0;  // is uL
    fieldsH[0] = uH0;  // is uH
    TimeStep dtL = dt;
    TimeStep dtH = dt;
    ...
    DNS dnsH(fieldsH, flags);
    DNS dnsL(fieldsL, flags);
    ...
    psolver.solve(fieldsH[1], fieldsH[0]);
    psolver.solve(fieldsL[1], fieldsL[0]);
        ...
        if (stateIsOnEdge) {
            ...
            if (l2d < etflags.epsilonAdvance) {
                ...
                if (t > etflags.tLastWrite) {  // prevent writing the same stuff twice
                    printout(fieldstats_t(fieldsH[0], t), fH);
                    printout(fieldstats_t(fieldsL[0], t), fL);
                }
                ecf2 = ecf1;
                ecf1 = ecf0;
                ecf0 = Ecf(fieldsH[0]);
 ...
 */
void ChooseNewLH(const FlowField& u0, Real& lambdaL, Real& lambdaH, const Real t, const EdgetrackingFlags& etflags,
                 const AttractorFlags& aflags, const TimeStep& dt, const DNSFlags& dnsflags, ostream& os) {
    ...
    while (AH != Turbulent) {
        u = u0;
        u *= lambdaH;
        AH = CalcAttractor(u, aflags, dt, dnsflags,
    ...
    while (AL != Laminar) {
        ...
        AL = CalcAttractor(u, aflags, dt, dnsflags,
    ...
    FlowField uH(u0);
    uH *= lambdaH;
    SaveForContinuation(uL, uH, etflags.directoryBisections, t);
}

/** Save all data needed for continuation to file 'continue'
 */
void SaveForContinuation(const FlowField& uL, const FlowField& uH, const string directoryBisections, Real t) {
 ...
 */
Attractor CalcAttractor(const FlowField& u0, const AttractorFlags& aflags, const TimeStep& dt_, const DNSFlags& flags,
                        const string energyfile, const string saveFinalFF, const int tMin, const int tMax, bool verbose,
                        const int t0, ostream& os) {
    FlowField u(u0);
    return CalcAttractor(u0, u, aflags, dt_, flags, energyfile, saveFinalFF, tMin, tMax, verbose, t0, os);
}

Attractor CalcAttractor(const FlowField& u0, FlowField& u, const AttractorFlags& aflags, const TimeStep& dt_,
                        const DNSFlags& flags, const string energyfile, const string saveFinalFF, const int tMin,
    ...
    psolver.solve(fields[1], fields[0]);
    ofstream f;
    bool saveEnergy = (energyfile != "");
    if (saveEnergy) {
        if (u0.taskid() == 0)
            f.open(energyfile.c_str(), ios::out);
        printout(fieldstatsheader_t(), f);
        printout(fieldstats_t(u, t0), f);
    }
    ...
    while (result == NoAttractor && t < tMax) {
        ...
        if (saveEnergy) {
            printout(fieldstats_t(fields[0], t + t0), f);
        }
