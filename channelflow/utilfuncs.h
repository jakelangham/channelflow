...
void save(const std::string& filebase, cfbasics::Real T, const FieldSymmetry& tau);
void load(const std::string& filebase, cfbasics::Real& T, FieldSymmetry& tau);
...
FieldSymmetry optimizePhase(const FlowField& u, const FieldSymmetry& s, int Nsteps = 10, cfbasics::Real residual = 1e-13,
                            cfbasics::Real damp = 1.0, bool verbose = true, cfbasics::Real x0 = 0.0, cfbasics::Real z0 = 0.0);
...
cfbasics::Real optPhaseShiftx(const FlowField& u0, const FlowField& u1, cfbasics::Real amin, cfbasics::Real amax, int nSampling = 3,
                    cfbasics::Real tolerance = 1e-3);
...
FieldSymmetry xfixphasehack(FlowField& u, cfbasics::Real axguess = 0.0, int i = 0, cfbasics::parity p = cfbasics::Odd, std::string mode = "std");
...
FieldSymmetry zfixphasehack(FlowField& u, cfbasics::Real azguess = 0.0, int i = 0, cfbasics::parity p = cfbasics::Odd, std::string mode = "std");
...
cfbasics::Real PuFraction(const FlowField& u, const FieldSymmetry& s, int sign);
...
class FieldSeries {
   public:
    ...
    void push(const FlowField& f, cfbasics::Real t);
    void interpolate(FlowField& f, cfbasics::Real t) const;
    bool full() const;

   private:
    cfbasics::cfarray<cfbasics::Real> t_;
    cfbasics::cfarray<FlowField> f_;
...
cfbasics::Real tFromFilename(const std::string filename);
...
std::vector<ChebyCoeff> baseFlow(int Ny, cfbasics::Real a, cfbasics::Real b, DNSFlags& flags, std::string Uname, std::string Wname);
