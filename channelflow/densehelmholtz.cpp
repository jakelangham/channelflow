...
DenseHelmholtzSolver::DenseHelmholtzSolver()
    : N_(0), nModes_(0), nEvenModes_(0), nOddModes_(0), a_(0), b_(0), lambda_(0), nu_(0), Ae_(), Ao_(), Be_(), Bo_() {}

DenseHelmholtzSolver::DenseHelmholtzSolver(int numberModes, BoundaryCond bc, Real a, Real b, Real lambda, Real nu, bool normalise_mass)
    : N_(numberModes - 1),
      nModes_(numberModes),
      nEvenModes_(N_ / 2 + 1),
      nOddModes_(N_ / 2),
      ...
      lambda_(lambda),
      nu_(nu),
      normalise_mass_(normalise_mass),
      Ae_(nEvenModes_),
      Ao_(nOddModes_),
      Be_(nEvenModes_),
      Bo_(nOddModes_),
...
//void DenseHelmholtzSolver::verify(const ChebyCoeff& u, const ChebyCoeff& f, Real ua, Real ub, bool verbose) const {
//    ChebyTransform trans(nModes_);
//    assert(nModes_ == u.length());
//
//    ChebyCoeff u_yy = diff2(u);
...
//}
//
//void DenseHelmholtzSolver::solve(ChebyCoeff& u, Real& mu, const ChebyCoeff& f, Real umean, Real ua, Real ub) const {
...
//}
//
//void DenseHelmholtzSolver::verify(ChebyCoeff& u, Real& mu, const ChebyCoeff& f, Real umean, Real ua, Real ub) const {
...
//}
//
//Real DenseHelmholtzSolver::residual(const ChebyCoeff& u, Real mu, const ChebyCoeff& f, Real umean, Real ua, Real ub) const {
//    ChebyCoeff rhs(f);
//    rhs[0] += mu;
...
Real DenseHelmholtzSolver::lambda() const { return lambda_; }
