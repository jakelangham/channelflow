...
void cfDSI::updateMu(Real mu) {
    ...
    if (cPar_ == continuationParameter::Re) {
        ...
        dnsflags_.vs = mu;
    } else if (cPar_ == continuationParameter::kappa) {
        dnsflags_.kappa = mu;
...
void cfDSI::chooseMu(continuationParameter mu) {
    ...
    switch (mu) {
        ...
        case continuationParameter::vs:
            updateMu(dnsflags_.vs);
            break;
        case continuationParameter::kappa:
            updateMu(dnsflags_.kappa);
            break;
...
continuationParameter cfDSI::s2cPar(string muname) {
    ...
    else if (muname == "kappa")
        return continuationParameter::kappa;
    else if (muname == "T")
...
string cfDSI::cPar2s(continuationParameter cPar) {
    if (cPar == continuationParameter::Re)
        return "Re";
    else if (cPar == continuationParameter::Pr)
        return "Pr";
