...
#ifndef CHANNELFLOW_DENSEHELMHOLTZ_H
#define CHANNELFLOW_DENSEHELMHOLTZ_H
...
// DenseHelmholtzSolver solves the 1d Helmholtz eqn:  u'' - lambda u = f on [a,b]
...
class DenseHelmholtzSolver {
   public:
    DenseHelmholtzSolver();
...
//    Real residual(const ChebyCoeff& u, Real mu, const ChebyCoeff& f, Real umean, Real ua, Real ub) const;
   ...
   private:
    ...
    Real lambda_;
    Real nu_;
    bool normalise_mass_; // enforce that soln to integrates to 1
...
// inline int DenseHelmholtzSolver::beta(int n) const {return (n > N_-2) ? 0 : 1;}
inline int DenseHelmholtzSolver::beta(int n) const { return (n > N_ - 2) ? 0 : 1; }
inline int DenseHelmholtzSolver::c(int n) const { return (n == 0 || n == N_) ? 2 : 1; }
